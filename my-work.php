<?php require_once('header.php');?> 

<section id="my-work">
    <div class="container">
       <div class="row">
           <div class="col-lg-12 text-center">
               <h2>
                   <span class="italic">Done by : </span><span class="uppercase">Me</span>
               </h2>
           </div>
       </div>
        <div class="row">
            <div class="col-lg-4 col-md-6 col-sm-6">
                <ul>
                    <li>
                        <a id="modal01" href="#modal-01" class="portfolio-link"><h3><span class="bold">Bathys : </span><span class="italic">des solutions d'inspection & d'assistance sur mesure</span></h3></a>
                        
                        <img src="img/bathys/bathys-intro.jpg" alt="Bathys" class="img-responsive">
                        
                        <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Cupiditate voluptas vitae repellat id adipisci beatae illo, molestiae mollitia, laborum sit pariatur fugiat. Qui dicta obcaecati nisi, architecto magni soluta fuga?</p>
                    </li>
                    <li>
                        <a id="modal02" href="#modal-02" class="portfolio-link"><h3><span class="bold">Inspiration : </span><span class="italic">printemps, animaux, fleurs... </span></h3></a>
                        
                        <img src="img/inspiration/inspiration-animals.jpg" alt="Blog Monde de Noah" class="img-responsive">
                        
                        <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Cupiditate voluptas vitae repellat id adipisci beatae illo, molestiae mollitia, laborum sit pariatur fugiat. Qui dicta obcaecati nisi, architecto magni soluta fuga?</p>
                    </li>
                </ul>
            </div>
               
            <div class="col-lg-4 col-md-6 col-sm-6">
                <ul>
                    <li>
                        <a id="modal03" href="#modal-03" class="portfolio-link"><h3><span class="bold">Monde de Noah : </span><span class="italic">monde de vélos, livres, jeux games, films... </span></h3></a>

                        <img src="img/mondedenoah/mondedenoah-intro.jpg" alt="" class="img-responsive">

                        <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Cupiditate voluptas vitae repellat id adipisci beatae illo, molestiae mollitia, laborum sit pariatur fugiat. Qui dicta obcaecati nisi, architecto magni soluta fuga?</p>
                        <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Cupiditate voluptas vitae repellat id adipisci beatae illo, molestiae mollitia, laborum sit pariatur fugiat. Qui dicta obcaecati nisi, architecto magni soluta fuga?</p>
                    </li>
                    <li>
                        <a id="modal04" href="#modal-04" class="portfolio-link"><h3><span class="bold">Michel photographie : </span><span class="italic">somehing is going wrong and it needs to be fixed</span></h3></a>

                        <img src="img/my-time-bg.jpg" alt="" class="img-responsive">

                        <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Cupiditate voluptas vitae repellat id adipisci beatae illo, molestiae mollitia, laborum sit pariatur fugiat. Qui dicta obcaecati nisi, architecto magni soluta fuga?</p>
                    </li>
                </ul>
            </div>
               
            <div class="col-lg-4 col-md-12 col-sm-12">
                <ul>
                    <li class="col-lg-12 col-md-6 col-sm-6">
                        <a id="modal05" href="#modal-05" class="portfolio-link"><h3><span class="bold">EDF : </span><span class="italic">equipe de France depuis 1976</span></h3></a>

                        <img src="img/EDF/edf-intro.jpg" alt="Equipe de France" class="img-responsive">

                        <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Cupiditate voluptas vitae repellat id adipisci beatae illo, molestiae mollitia, laborum sit pariatur fugiat. Qui dicta obcaecati nisi, architecto magni soluta fuga?</p>
                    </li>
                    <li class="col-lg-12 col-md-6 col-sm-6">
                        <a id="modal06" href="#modal-06" class="portfolio-link"><h3><span class="bold">La Linda : </span><span class="italic">somehing is going wrong and it needs</span></h3></a>

                        <img src="img/my-time-bg.jpg" alt="" class="img-responsive">

                        <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Cupiditate voluptas vitae repellat id adipisci beatae illo, molestiae mollitia, laborum sit pariatur fugiat. Qui dicta obcaecati nisi, architecto magni soluta fuga?</p>
                    </li>
                </ul>
            </div>
        </div><!-- row -->
    </div><!--continer-->
</section>

<?php require_once('modal.php');?>  

<?php require_once('footer.php');?>  